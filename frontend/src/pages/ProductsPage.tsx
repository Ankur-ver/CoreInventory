      ...
      {loading ? (
        <div className="grid grid-cols-4 gap-4">
          {Array.from({ length: 8 }).map((_, i) => (
            <div key={i} className="h-44 bg-bg-surface animate-pulse rounded-lg" />
          ))}
        </div>
        ...
        <div className="grid grid-cols-4 gap-4">
          {displayProducts.map((p) => {
            ...
            // Build zone breakdown for this product
            ...
            return (
              <div
                key={p.id}
                className="card p-4 cursor-pointer hover:border-border-strong hover:-translate-y-px transition-all duration-200 group flex flex-col"
              ...
              >
                ...
                {/* Name */}
                <div className="font-head font-semibold text-[14px] text-text-primary mb-3 leading-tight group-hover:text-accent transition-colors flex-1">
                  {p.name}
                </div>
                ...
                <div className="flex items-end justify-between mb-3">
                  ...
                  <div className="text-right">
                    ...
                    <div className="text-[10px] text-text-muted uppercase tracking-wide">Price</div>
                  </div>
                </div>
                ...
                {zoneBreakdown.length > 0 ? (
                  <div className="border-t border-border pt-2.5 mt-auto">
                    <div className="text-[10px] text-text-muted uppercase tracking-wide mb-1.5 font-medium">Stored in</div>
                    <div className="space-y-1">
                      ...
                      {zoneBreakdown.length > 2 && (
                        <div className="text-[10px] text-text-muted">+{zoneBreakdown.length - 2} more zone{zoneBreakdown.length - 2 !== 1 ? 's' : ''}</div>
                      )}
                    </div>
                  </div>
                ) : (
                    ...
                    <div className="h-1 bg-bg-surface3 rounded-full overflow-hidden">
                      <div
                        className={`h-full rounded-full transition-all ${
                          worstStatus === 'OUT' ? 'bg-status-danger' :
                          worstStatus === 'LOW' ? 'bg-status-warn' : 'bg-status-success'
                        }`}
                        style={{ width: `${Math.min(100, (totalQty / (p.reorderPoint * 3)) * 100)}%` }}
                      />
                    </div>
...
function ProductModal({ product, categories, onClose, onSaved, onDelete }: {
  ...
  const [form, setForm] = useState({
    ...
    reorderPoint: product?.reorderPoint !== undefined ? product.reorderPoint : '',
  ...
  const handleSubmit = async () => {
    if (!form.name || !form.sku || !form.categoryId || form.reorderPoint === '' || form.price === '' || isNaN(Number(form.reorderPoint)) || isNaN(Number(form.price))) {
    ...
    <div className="fixed inset-0 bg-black/70 z-50 flex items-center justify-center p-4" onClick={(e) => e.target === e.currentTarget && onClose()}>
      <div className="bg-bg-surface border border-border-strong rounded-xl w-full max-w-md max-h-[90vh] overflow-y-auto shadow-2xl">
        ...
        <div className="px-6 py-5 space-y-4">
          ...
          <div className="grid grid-cols-2 gap-3">
            ...
            <div>
              ...
              <select className="select" value={form.unit} onChange={e => set('unit', e.target.value)}>
                {['pcs', 'kg', 'g', 'L', 'ml', 'm', 'cm', 'box', 'roll'].map(u => (
          ...
          <div className="grid grid-cols-2 gap-3">
            <div>
              ...
              <input type="number" className="input" value={form.reorderPoint}
                onChange={e => set('reorderPoint', e.target.value)} min={0} placeholder="Enter reorder point" />
            </div>
            <div>
              <label className="label">Price</label>
              <input type="number" className="input" value={form.price}
                onChange={e => set('price', e.target.value)} min={0} step="0.01" placeholder="Enter price" />
            </div>
          </div>
          <div>
          ...
          {/* Zone breakdown — only in edit mode */}
          {isEdit && zoneBreakdown.length > 0 && (
            <div>
                  ...
                  <div key={i} className="flex items-center justify-between bg-bg-surface2 border border-border rounded-lg px-3 py-2">
                    ...
                    <div className="text-right flex-shrink-0 ml-2">
                      <div className={`text-[13px] font-mono font-semibold ${
                        z.status === 'OUT' ? 'text-status-danger' :
                        z.status === 'LOW' ? 'text-status-warn' : 'text-status-success'
                      }`}>
                        {z.quantity} {z.unit}
                      </div>
              ...
              <div className="mt-2 text-[11px] text-text-muted">
                To add more stock to a zone, go to <span className="text-accent">Warehouses → Zone → Add Inventory</span>
              </div>
            </div>
          )}
          ...
          {isEdit && zoneBreakdown.length === 0 && (
            <div className="bg-bg-surface2 border border-dashed border-border rounded-lg p-3 text-center">
              ...
              <div className="text-[11px] text-text-muted">Go to <span className="text-accent">Warehouses → Zone → Add Inventory</span> to stock this product</div>
            </div>
          )}
        </div>
        ...
        <div className="flex gap-2 px-6 py-4 border-t border-border">
          {isEdit && (
            <button className="btn-danger text-xs px-3" onClick={() => { onDelete(product!.id); onClose(); }}>
              Delete
            </button>
          )}
